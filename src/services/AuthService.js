...
export default class AuthService {
  constructor(domain) {
    this.domain = domain || "https://salty-shelf-93882.herokuapp.com/"; // We can pass in the backend server, or use a default for dev
  ...
  isTokenExpired(token) {
    try {
      ...
      if (decoded.exp < 0) {
        //Replaced Date.now() / 1000 with 0 for development purposes
        return true;
      } else return false;
    } catch (err) {
      return false;
